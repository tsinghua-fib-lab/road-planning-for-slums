...
def shortest_path_setup(myA, p, roads_only=False):
    ...
    for i in myA.road_nodes:
        if len(list(myA.G.neighbors(i))) > 2:
...
def import_and_setup(filename,
    ...
    rezero_vector = np.array([xxmin, yymin])
    rescale_vector = np.ones(2)/max(myG1.myweight())

    myG2 = rescale_mygraph(myG1, rezero=rezero_vector, rescale=rescale_vector,xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax)
    print(max([myG1.G[e.nodes[0]][e.nodes[1]]['weight'] for e in myG1.myedges() if not e.road]))
    myG2.rezero_vector = rezero_vector
    myG2.rescale_vector = rescale_vector
    
    # myG2 = myG2.clean_up_geometry(0.1, err, byblock)
    print("geometery cleaned up")
...
def rescale_mygraph(myG, rezero=np.array([0, 0]), rescale=np.array([1, 1]),xmin=-100,xmax=100,ymin=-100,ymax=100):
    ...
    eall=myG.myedges()
    wall=myG.myweight()
    for idx in range(len(myG.myedges())):
        e=eall[idx]
        w=wall[idx]
        n0 = e.nodes[0]
        n1 = e.nodes[1]
        ...
        if (nn0.x > xmin and nn0.x < xmax and nn1.x > xmin and nn1.x < xmax and nn0.y > ymin and nn0.y < ymax and nn1.y > ymin and nn1.y < ymax):
            scaleG.add_edge(mg.MyEdge((nn0, nn1)), weight=w * rescale[0])
    return scaleG
