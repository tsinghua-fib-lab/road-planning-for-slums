...
from road_planning.models.state_encoder import SGNNStateEncoder, DGNNStateEncoder, NGNNStateEncoder, MLPStateEncoder, RMLPStateEncoder
from road_planning.models.policy import UrbanPlanningPolicy
from road_planning.models.value import UrbanPlanningValue
...
def create_dgnn_model(cfg, agent):
    """Create policy and value network from a config file.
    Args:
        cfg: A config object.
        agent: An agent.
    Returns:
        A tuple containing the policy network and the value network.
    """
    shared_net = DGNNStateEncoder(cfg.state_encoder_specs, agent)
    policy_net = UrbanPlanningPolicy(cfg.policy_specs, agent, shared_net)
    value_net = UrbanPlanningValue(cfg.value_specs, agent, shared_net)
    return policy_net, value_net


def create_ngnn_model(cfg, agent):
    """Create policy and value network from a config file.
    Args:
        cfg: A config object.
        agent: An agent.
    Returns:
        A tuple containing the policy network and the value network.
    """
    shared_net = NGNNStateEncoder(cfg.state_encoder_specs, agent)
    policy_net = UrbanPlanningPolicy(cfg.policy_specs, agent, shared_net)
    value_net = UrbanPlanningValue(cfg.value_specs, agent, shared_net)
    return policy_net, value_net
